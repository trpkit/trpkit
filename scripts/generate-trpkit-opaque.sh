...
# Build server
wasm-pack build --out-dir "$rootDir/autogenerated/opaque-server" --release --target nodejs "$rootDir/tmp/opaque-wasm"
if [ $? -ne 0 ]; then
...
# Build client
wasm-pack build --out-dir "$rootDir/autogenerated/opaque-client" --release --target web "$rootDir/tmp/opaque-wasm" --no-default-features --features client
if [ $? -ne 0 ]; then
